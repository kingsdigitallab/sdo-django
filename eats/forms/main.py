...
class SearchForm (forms.Form):
    authorities = [(a.id, a.authority) for a in Authority.objects.order_by('authority')]
    ...
    def clean (self):
        if not (self.data.get('name') or
            ...
            raise forms.ValidationError('Either a name or an authority with either an ID or URL is required.')
